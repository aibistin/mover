...
sub index : Path : Args(0) {
    my ($self, $c) = @_;
    DEBUG '*** We got to Menu from  ' . $c->request->path;

        #------ Page Heading
    $c->stash(h2_main_heading  => "Main Menu",
...
sub estimates_menu : Path('estimates_menu') {
    ...
    #------ Page Heading
    $c->stash(h2_main_heading  => "Estimates Department",
              main_sub_heading => "Schedule, Modify And View Estimates",
    );
    
    DEBUG '*** INSIDE menu estimates_menu METHOD';
