...
use mover::Schema;
#use mover::Auth::Schema;

#-------------------------------------------------------------------------------
#  Change this to reflect the User table in the mover.db
#  AK Sep 17 2012
#-------------------------------------------------------------------------------
my $user = undef;
my $passwd = undef;
...
my $role = undef;
my $email = undef;
my $schema = mover::Auth::Schema->connect("dbi:SQLite:$Bin/../db/auth.db");
...
mover_auth_admin.pl - Sets the username and password for the generated authentication database

=head1 SYNOPSIS

mover_auth_admin.pl -user username -passwd password [-role role]
